...
;   macro     X     Y        "translation"               ; English original
    ...
    TCODE    .100, .50,      "DTiefeL�"                  ;033 ResetAvr          (Reset average depth)
...
; 32 custom function descriptors I (FIXED LENGTH = 15 chars).
    ...
    TCODE    .40,  .35,      "TG Men�s  [min]"           ;046 Dive menus[min]
    TCODE    .40,  .35,      "S�ttigung x [%]"           ;047 Saturate x  [%]
    TCODE    .40,  .35,      "Ents�tt. x  [%]"           ;048 Desaturate x[%]
...
;licence:
	...
	TCODE    .0,   .95,      "gestellt, dass es n�tz"    ;070 hope that it will be
    ...
    TCODE    .0,   .35,      "oder Eignung f�r einen"    ;075 MERCHANTABILITY or
...
;
    ...
    TCODE    .14,   .2,      "Einstellungs-Men�:"        ;098 Setup Menu:
    ...
    TCODE    .20,   .65,     "CCR Setup Men�"            ;110 CCR Setup Menu
    TCODE    .28,   .2,      "CCR Setup Men�"            ;111 CCR Setup Menu
    TCODE    .0,    .0,      "SP#"                       ;112 SP#
...
; 32 custom function descriptors II (FIXED LENGTH = 15 chars).
    ...
    TCODE    .40,   .35,     "Festen SP �nde."           ;161 Adjust fixed SP
    ...
    TCODE    .40,   .35,     "H�henmesser anz"           ;171 Show altimeter
...
; 32 custom function descriptors III (FIXED LENGTH = 15 chars).
    ...
    TCODE    .40,   .35,     "GF Wechsel m�gl"           ;198 Allow GF change
    TCODE    .40,   .35,     "S.Stop L�nge[s]"           ;199 S.StopLength[s] (CF70: Safety Stop Duration [s])
...
;
    ...
    TCODE    .10,   .2,      "Dekomodell ver�ndert!"     ;235 Decomode changed!
    ...
    TCODE    .20,   .65,     "CF,Gas&Deko zur�cks."      ;245 Reset CF,Gas & Deco
...
;
    ...
    TCODE    .10,   .2,      "Einstellungs-Men� 2:"      ;258 Setup Menu 2:
...
; New CFs Warning
    TCODE    .10,   .2,      "Neue CF hinzugef�gt!"      ;271 New CF added!
    TCODE    .0,    .35,     "Neue Custom Funktionen"    ;272 New CustomFunctions
    TCODE    .0,    .65,     "wurden hinzugef�gt!"       ;273 were added! Check
    TCODE    .0,    .95,     "Pr�fe CF I - CF III"       ;274 CF I - CF III Menu
    TCODE    .0,    .125,    "Men� f�r Details!"         ;275 for Details!
...
;
    ...
    TCODE    .20,   .95,     "Logbuch zur�cks."          ;284 Reset Logbook
    TCODE    .20,   .125,    "OSTC neu starten"          ;285 Reboot OSTC
    TCODE    .20,   .155,    "Dekodaten zur�cks."        ;286 Reset Decodata
; Altimeter extension
    TCODE    .20,   .155,    "H�henmesser"               ;287 Altimeter
    TCODE    .18,   .1,      "H�henmesser einst."        ;288 Set Altimeter
    TCODE    .20,   .35,     "H�he NN: "                 ;289 Sea ref:
...
; ZH-L16 mode description
    ...
    TCODE    .0,    .65,     "F�r das Tauchen mit"       ;315 For Open Circuit
    TCODE    .0,    .95,     "offenen Systemen. "        ;316 Divers. Supports 5
    TCODE    .0,    .125,    "Bis zu 5 Trimix Gase."     ;317 Trimix Gases.
    TCODE    .0,    .155,    "Hierzu das Men� Gas-"      ;318 Configure your gas
    TCODE    .0,    .185,    "einstellung verwenden."    ;319 in Gassetup menu.
    TCODE    .0,    .215,    "CF11 & CF12 pr�fen!"       ;320 Check CF11 & CF12 !
...
; Apnoemode description
    ...
    TCODE    .0,    .155,    "rate wird tempor�r auf"    ;339 Will temporally set
...
; Multi GF OC mode description
    ...
    TCODE    .0,    .155,    "Pr�fe die CF32 & CF33"     ;346 _hi). Check CF32 &
    TCODE    .0,    .185,    "Werte! F�r offene Sys-"    ;347 CF33! Open Circuit
...
; Multi GF CC mode description
    ...
    TCODE    .0,    .125,    "(GF_lo/GF_hi). Pr�fe"      ;352 GF-Method (GF_lo/GF
    TCODE    .0,    .155,    "die CF32 & CF33 Werte!"    ;353 _hi). Check CF32 &
    TCODE    .0,    .185,    "F�r geschlossene Sys-"     ;354 CF33!Closed Circuit
...
; pSCR-GF mode description
    ...
    TCODE    .0,    .65,     "F�r passive halb-"     	;357 For passive semi-
    TCODE    .0,    .95,     "geschlossene Kreisel."   	;358 closed rebreather.
    TCODE    .0,    .125,    "Pr�fe CF32 & CF33"     	;359 Check CF32 & CF33
    TCODE    .0,    .155,    "f�r Gradienten Faktoren" 	;360 for gradient factors
    TCODE    .0,    .185,    "und CF61-CF63 f�r"     	;361 and CF61-CF63 for
